import json
from typing import Dict, Any, List

class LLMPlanner:
    """Simulates an LLM-based planner that converts user instructions and UI state into action plans."""

    def __init__(self):
        # In a real scenario, this would initialize an LLM client (e.g., OpenAI, Gemini)
        pass

    def generate_action_plan(self, instruction: str, ui_state: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Generates a simulated action plan based on user instruction and current UI state.

        Args:
            instruction (str): The user's natural language instruction (e.g., "Login to the app").
            ui_state (Dict[str, Any]): The structured UI state from visual perception.

        Returns:
            List[Dict[str, Any]]: A list of simulated actions.
        """
        print(f"[LLMPlanner] Generating plan for instruction: \'{instruction}\' with UI state...")
        # This is a highly simplified simulation. A real LLM would perform complex reasoning.
        # We'll use keywords to trigger different mock plans.

        action_plan = []
        if "login" in instruction.lower():
            action_plan = [
                {"action": "type_text", "target_id": "username_field", "value": "testuser"},
                {"action": "type_text", "target_id": "password_field", "value": "testpassword"},
                {"action": "tap", "target_id": "login_button"},
                {"action": "wait_for_displayed", "target_id": "home_screen_element"},
            ]
        elif "profile" in instruction.lower() or "navigate" in instruction.lower():
            action_plan = [
                {"action": "tap", "target_id": "profile_button"},
            ]
        else:
            print("[LLMPlanner] No specific plan found for instruction. Returning empty plan.")

        print(f"[LLMPlanner] Generated plan: {json.dumps(action_plan, indent=2)}")
        return action_plan

    def reflect_and_correct(self, instruction: str, ui_state: Dict[str, Any], failed_action: Dict[str, Any], error_message: str) -> List[Dict[str, Any]]:
        """Simulates LLM reflection to correct a failed action plan."
        print(f"[LLMPlanner] Reflecting on failed action: {failed_action} with error: {error_message}")
        print("[LLMPlanner] Attempting to generate a corrective plan...")

        # In a real LLM, this would involve analyzing the error and UI state to propose a new approach.
        # For simulation, we'll just try to re-tap the login button if it was a login failure.
        if failed_action.get("target_id") == "login_button" and "not displayed" in error_message.lower():
            print("[LLMPlanner] Suggesting re-tapping login button after a short delay.")
            return [
                {"action": "sleep", "value": 2},
                {"action": "tap", "target_id": "login_button"},
                {"action": "wait_for_displayed", "target_id": "home_screen_element"},
            ]
        else:
            print("[LLMPlanner] No specific corrective action simulated. Returning empty plan.")
            return []

# Example usage (for testing purposes)
if __name__ == '__main__':
    planner = LLMPlanner()
    mock_ui_state = {
        "timestamp": "20251007_123456",
        "screenshot_path": "mock_screenshot.png",
        "full_text_ocr": "Welcome to the app. Username Password Login",
        "ui_elements": [
            {"text": "Username", "bbox": [50, 100, 150, 120], "type": "label"},
            {"text": "", "bbox": [160, 95, 300, 125], "type": "input", "id": "username_field"},
            {"text": "Password", "bbox": [50, 150, 150, 170], "type": "label"},
            {"text": "", "bbox": [160, 145, 300, 175], "type": "input", "id": "password_field"},
            {"text": "Login", "bbox": [100, 200, 250, 230], "type": "button", "id": "login_button"},
            {"text": "Welcome to Dashboard", "bbox": [50, 50, 350, 80], "type": "text", "id": "home_screen_element"},
        ],
        "device_info": {"deviceName": "MockDevice", "platformName": "mockOS"}
    }

    print("\n--- Test 1: Login Instruction ---")
    plan = planner.generate_action_plan("Login to the application", mock_ui_state)
    print(json.dumps(plan, indent=2))

    print("\n--- Test 2: Navigate to Profile Instruction ---")
    plan = planner.generate_action_plan("Go to my profile", mock_ui_state)
    print(json.dumps(plan, indent=2))

    print("\n--- Test 3: Reflection and Correction ---")
    failed_action = {"action": "tap", "target_id": "login_button"}
    error_msg = "Element login_button not displayed in 10s"
    corrective_plan = planner.reflect_and_correct("Login to the application", mock_ui_state, failed_action, error_msg)
    print(json.dumps(corrective_plan, indent=2))

